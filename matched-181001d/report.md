...
- After: `181001d4b48d829fd9b138cf34cdd838555ba160` (PR head)
...
- **Severity stopped defaulting to `Error`.** Android's `InlineNotice` declared `severity: NoticeSeverity = NoticeSeverity.Error`, so 24 of 44 call sites rendered the loudest tier by not mentioning it. Severity is now required on both platforms and every call site states a tier chosen by what the message costs the user. Six sites moved off `Error`: both scanner overlays, NFC-unavailable (to `Info`), NFC-disabled, and the two status-check failures. Visible in the variants pair as the scanner overlay turning from the red error container to the amber caution one.
